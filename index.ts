...
let tasks: Task[] = JSON.parse(localStorage.getItem('kanban_tasks') || '[]');

function saveTasksToStorage(): void {
  localStorage.setItem('kanban_tasks', JSON.stringify(tasks));
}

let modal = document.getElementById('task-modal') as HTMLDivElement;
let openModalBtn = document.getElementById('open-modal-btn') as HTMLButtonElement;
let closeModalBtn = document.getElementById('close-modal-btn') as HTMLButtonElement;
let cancelModalBtn = document.getElementById('cancel-modal-btn') as HTMLButtonElement;
let taskForm = document.getElementById('task-form') as HTMLFormElement;

let taskIdInput = document.getElementById('task-id') as HTMLInputElement;
let taskTitleInput = document.getElementById('task-title') as HTMLInputElement;
let taskPriorityInput = document.getElementById('task-priority') as HTMLSelectElement;
let taskDateInput = document.getElementById('task-date') as HTMLInputElement;
let taskDescInput = document.getElementById('task-desc') as HTMLTextAreaElement;
let charCount = document.getElementById('char-count') as HTMLDivElement;

let modalTitle = document.getElementById('modal-title') as HTMLHeadingElement;
...
function openModal(isEdit = false): void {
...
taskForm.addEventListener('submit', (e: Event) => {
  ...
  let priority = taskPriorityInput.value as Task['priority'];
...
function moveTask(id: string, newStatus: Task['status']): void {
  let task = tasks.find((t) => t.id === id);
  if (task) {
